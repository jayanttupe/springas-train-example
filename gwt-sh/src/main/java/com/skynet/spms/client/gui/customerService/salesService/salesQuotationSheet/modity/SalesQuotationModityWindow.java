...
public class SalesQuotationModityWindow extends BaseWindow {
	...
	protected Canvas getLeftLayout(Rectangle srcRect, ListGrid listGrid) {
		...
		//修改报价单
		this.setTitle(ui.getB().modifyFormTitle(ui.getM().mod_salesQuotationSheet_name()));
		/** 主面板* */
	...
	private VLayout getPrimaryView() {
		...
		item_5.setTitle(ui.getM().title_CustomerIdentificationCode());
	...
	private HLayout getShowGridView() {
		...
		Label leftLb = new Label(ui.getB().msgListTitle(ui.getM().mod_salesInquiry_item_name()));
		leftLb.setHeight("20");
		...
		Label rightLb = new Label(ui.getB().msgListTitle(ui.getM().mod_salesQuotationSheet_item_name()));
	...
	private VLayout getItemView() {
		...
		fi19.addFormItemClickHandler(new FormItemClickHandler(){
			...
			public void onFormItemClick(FormItemIconClickEvent event) {
				if(!itemId.equals("")){
					SalesPiecewiseQuotationWindow win = new SalesPiecewiseQuotationWindow(ui.getM().mod_salesQuotationSheet_fdbtn_title(),false,null,null,null);
					...
					//请先保存明细
					SC.say(ui.getB().msgFirstSaveItem());
				}
			}
