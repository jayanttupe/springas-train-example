...
public class ContractBaseForm extends VLayout {
	...
	public ContractBaseForm() {
		form.setDataSource(model.repairContractListGrid.getDataSource());
		form.editSelectedData(model.repairContractListGrid);
		build();
	}
	...
	private void build() {
		...
		carrierNameItem.setTitle(ui.getRepairModule().title_for_shipmanager());
		...
		contractItemSection.setDefaultValue(ui.getRepairModule().repair_item_title());
		...
		form.setFields(contratNumberItem, planeLastNumberItem, priorityItem,
				...
				makeByItem, /*startDateItem, endDateItem,
				extendedValueTotalAmountItem,*/ contractItemSection,
		...
		final IButton btnSave = new IButton(ui.getB().btnSaveModify());
		...
		btnClose.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				Window win = (Window) Cache.getInstance().getObject(
						"RepairContractTemplateUpdateWin");
				if (win != null) {
					win.destroy();
				}
