...
public class ContractItemAddForm extends VLayout {
	...
	public ContractItemAddForm(String groupTitle) {
		this.setHeight(350);
		this.setWidth100();
	...
	private void build() {
		...
		item_save.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (res >= 0 && state == DSOperationType.ADD) {
					//"不能添加明细信息，请核对后再试！"
					SC.say(ui.getM().msgCanNotSaveItem());
					return;
				}
		...
		form.setItems(item_no, item_ManufacturerCode, item_ManufacturerCodeId,
				...
				item_type, item_num, item_count, item_pakageNo, item_unit,
				item_unitPrice, item_currency, item_price, item_remark, item4Id);
		this.addMember(form);
