 ...
 * 
 */
public class SSLeaseContractAddWindow extends SuperWindow {
	...
	public SSLeaseContractAddWindow() {
		//新建租赁合同
		this.setTitle(ui.getM().addFormTitle(ui.getM().mod_ssLeaseContract_name()));
	...
	private ListGrid getLeftGrid() {
		...
		dataSourceTool
				.onCreateDataSource(
						com.skynet.spms.client.gui.customerService.common.DSKey.C_LEASECONTRACT,
						com.skynet.spms.client.gui.customerService.common.DSKey.C_LEASECONTRACT_TIEM_DS,
		...
		dataSourceTool.onCreateDataSource(DSKey.S_LEASECONTRACT,
				DSKey.S_LEASECONTRACTITEM_DS, new PostDataSourceInit() {
					public void doPostOper(DataSource dataSource,
							DataInfo dataInfo) {
						lg.setDataSource(dataSource);
						modelLocator.SSleaseContractItemDs = dataSource;
	...
	private VLayout getProcurementItemView() {
		...
		dataSourceTool.onCreateDataSource(DSKey.S_LEASECONTRACT,
				DSKey.S_LEASECONTRACTITEM_DS, new PostDataSourceInit() {
					public void doPostOper(DataSource dataSource,
						...
						leaseContractId.setName("ssleaseContractTemplate.id");
						...
						item1.addChangedHandler(new ChangedHandler() {
							public void onChanged(ChangedEvent event) {
								...
								item8.setValue(infoVO
										.getSuitableAircraftModel());
							}
						});
						// 制造商
						...
						item9.addChangedHandler(new ChangedHandler() {
							public void onChanged(ChangedEvent event) {
								item21.setValue(Float.parseFloat(item17
										.getValue().toString())
										* Integer.parseInt(item16.getValue()
												.toString())
										* Integer.parseInt(item9.getValue()
												.toString())
										+ Float.parseFloat(item19.getValue()
												.toString()));
							}
						});
						...
						item13.addBlurHandler(new BlurHandler() {
							public void onBlur(BlurEvent event) {
								long starttime = item15.getValueAsDate()
										.getTime();

								long endtime = item13.getValueAsDate()
										.getTime();
						...
						itemLdf.setFields(leaseContractId, item1, item2, item3,
								item4, item5, item8, item9, item10, item11,
								item12, item13, item15, item16, item17, item18,
								item19, item21, item22, item14);
					}
				});
		...
		// 构建操作按钮
		modelLocator.saveBtn.setDisabled(true);
		modelLocator.saveBtn.addClickHandler(new ClickHandler() {
		...
		itemBtnsLayout.setMembers(modelLocator.saveBtn, closeBtn);
