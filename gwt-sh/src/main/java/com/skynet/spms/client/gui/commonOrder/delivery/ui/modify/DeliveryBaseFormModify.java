...
public class DeliveryBaseFormModify extends VLayout {
	...
	public DeliveryBaseFormModify() {
		...
		baseForm.setDataSource(DataModelLocator.getInstance().modifyOrderGrid
				.getDataSource());
		addressForm.setDataSource(DataModelLocator.getInstance().modifyOrderGrid
				.getDataSource());
		build();
		buildAddressForm();
	...
	private void initButtonOpt() {
		...
		final IButton btnSave = new IButton(ui.btnSaveModify());
		btnSave.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				vm.saveData(new DSCallback() {
					public void execute(DSResponse response, Object rawData,
							DSRequest request) {
						SC.say(ui.msgModifyOpSuccess());
					}
				});
			}
		});
