 ...
 * 
 */
public class SalesInquiryAddByPartInquiryWindow extends BaseWindow {
	...
	private FilterListGrid waitItemListGrid;// 等待询价的明细列表
	private FilterListGrid itemListGrid;// 已经询价单的明细列表
	 ...
	 *            当前操作方式
	 */
	public SalesInquiryAddByPartInquiryWindow(String windowTitle,
	...
	protected Canvas getLeftLayout(Rectangle srcRect, ListGrid listGrid) {
		...
		// 新增询价单
		this
				.setTitle(ui.getB().addFormTitle(
						ui.getM().mod_salesInquiry_name()));// 新建询价单
		/** 主面板* */
	...
	private VLayout getProcurementView() {
		...
		v.setHeight100();
		// 询价单
		v.setGroupTitle(ui.getM().mod_salesInquiry_name());
		v.setIsGroup(true);
		...
		// 初始化联系人，联系方式
		CodeRPCTool.bindCustomerData(item_2.getValue() + "", item_3, item_5);
		...
		// 构建操作按钮
		final IButton saveBtn = new IButton(ui.getB().btnSave());// 保存
		final IButton cancelBtn = new IButton(ui.getB().btnClose());// 关闭
		// 保存
			...
			public void onClick(ClickEvent event) {
				ldf.saveData(new DSCallback() {
					public void execute(DSResponse response, Object rawData,
							DSRequest request) {
						SC.say(ui.getB().msgAddOpSuccess());// 保存成功
	...
	private HLayout getShowGridView() {
		...
		Label leftLb = new Label(ui.getB().msgListTitle(
				ui.getM().mod_salesInquiry_item_waitAdd()));
		leftLb.setHeight("20");
		...
		Label rightLb = new Label(ui.getB().msgListTitle(
				ui.getM().mod_salesInquiry_item_name()));
		rightLb.setHeight("20");
		rightLayout.addMember(rightLb);// 先放label
		rightLayout.addMember(getRightGrid());// 再放Grid

		h.setMembers(leftLayout, rightLayout);
		return h;
	}
	...
	private VLayout getProcurementItemView() {
		...
		v.setHeight100();
		// 询价单明细
		v.setGroupTitle(ui.getM().mod_salesInquiry_item_name());
		v.setIsGroup(true);
		...
		itemLdf.setFields(primaryKeyItem, item1, keywordItem, ataItem, item3,
				item2, item4, item5, item6, item7, item8, itemNumber);
				...
				if (itemLdf.validate()) {
					itemLdf.saveData(new DSCallback() {
						public void execute(DSResponse response,
							...
							waitItemListGrid.removeData(waitItemListGrid.getSelectedRecord());
						}
					});
				}
		...
		cleanBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				itemLdf.editNewRecord();
				Utils.makeAllNotSelectLG(waitItemListGrid);
			}
	...
	private ListGrid getLeftGrid() {
		waitItemListGrid = new FilterListGrid();
		waitItemListGrid.setRemoveFieldTitle(ui.getB().btnRemove());

		Utils.setListGridHeight(waitItemListGrid);

		// 询价件号
		ListGridField field1 = new ListGridField("partNumber",ui.getM().title_partNumber());
		// 关键字
		ListGridField field2 = new ListGridField("m_BasicInformation.keyword_zh",ui.getM().title_keyword());
		//制造商
		ListGridField field3 = new ListGridField("m_ManufacturerCode.code",ui.getM().title_manufacturer());
		
		field1.setCanFilter(true);
		field2.setCanFilter(true);
		field3.setCanFilter(true);

		waitItemListGrid.setFields(field1, field2,field3);

		ListGridRecord[] seleRecords = modelLocator.seleRecords;
		waitItemListGrid.setData(seleRecords);

		// 选中行事件
		waitItemListGrid.addCellClickHandler(new CellClickHandler() {
			public void onCellClick(CellClickEvent event) {
				...
				ListGridRecord selRecord = event.getRecord();
				item1.setValue(selRecord.getAttribute("partNumber"));
				service.getPartInfoByPartNumber(item1.getValueAsString(),
						new AsyncCallback<PartInfoVO>() {
							...
							public void onSuccess(PartInfoVO partInfoVO) {
								// TODO Auto-generated method stub
								ataItem.setValue(partInfoVO.getAtaNumber());
								item2.setValue(partInfoVO
										.getManufacturerCodeId());
								keywordItem.setValue(partInfoVO.getKeyword());
								item5.setValue(partInfoVO
	...
	private ListGrid getRightGrid() {
		...
		itemListGrid.setRemoveFieldTitle(ui.getB().btnRemove());
