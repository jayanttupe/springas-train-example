...
public class BuyBackSheetItemForm extends VLayout {
...
//	public SelectItem item_pakageNo;
	/** 数量 **/
	public TextItem item_count;
	/** 单位下拉列表 */
	public SelectItem item_unit;
	/** 单价 */
	public TextItem item_unitPrice;
	/** 币种 **/
	public SelectItem item_currency;
	/** 金额输入框 */
	public TextItem item_price;
	/** 备注 */
	...
	public BuyBackSheetItemForm(String groupTitle) {
		this.setHeight(400);
		this.setWidth100();
	...
	private void build() {
		 ...
		 * 初始化清空按钮
		 */
		item_new = new IButton(ui.getM().btnClean());
		item_new.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				form.editNewRecord();
				item_save.setTitle(ui.getM().btnSaveItem());
			}
		});
		/**
		 * 初始化关闭按钮
		 */
		item_close = new IButton(ui.getM().btnClose());
		 ...
		 * 初始化保存按钮
		 */
		item_save = new IButton(ui.getM().btnSaveItem());
		item_save.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				if (model.sheetID == null) {
					//"请先保存申请单信息！"
					SC.say(ui.getM().msgFirstSaveSheet());
					return;
				}
				sumAmount();
		 ...
		 * 初始化金额输入框
		 */
		item_price = new TextItem("price"/* , "金额" */);
		/**
		 * 初始化币种下拉列表
		 */
		 ...
		 * 初始化单价输入框
		 */
		item_unitPrice = new TextItem("unitPriceAmount"/* , "单价" */);
		 ...
		 * 初始化数量输入框
		 */
		item_count = new TextItem("quantity"/* , "数量" */);
