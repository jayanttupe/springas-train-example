 ...
 * 
 */
public class ConsignRenewUpdateWin extends BaseWindow {
	private static I18n ui = new I18n();
	public ConsignRenewUpdateWin(String windowTitle, boolean isMax,
	...
	public ConsignRenewItemUpdateForm itemForm;

	@Override
	protected Canvas getLeftLayout(Rectangle srcRect, ListGrid listGrid) {
		//修改寄售补库
		this.setTitle(ui.getM().modifyFormTitle(ui.getM().consignRenew_name()));
		...
		// 寄售补库
		Tab tab_Base = new Tab(ui.getM().consignRenew_name());
		tab_Base.setPane(new ConsignRenewBaseUpdateForm(consignItemGrid));
		topTabSet.addTab(tab_Base);
		// 收发地址
		Tab tab_Addr = new Tab(ui.getM().address_info_title());
		...
		//寄售协议明细列表
		protocolItemGrid.setGroupTitle(ui.getM().msgListTitle(ui.getM().consignProtocol_item_name()));
		...
		protocolGrid.addRecordClickHandler(new RecordClickHandler() {
			...
			public void onRecordClick(RecordClickEvent event) {
				...
				if (record != null) {
					...
					itemForm.item_save.setTitle(ui.getM().btnSaveItem());
		...
		//寄售补库明细列表
		renewItemGrid.setGroupTitle(ui.getM().msgListTitle(ui.getM().consignRenew_item_name()));
		...
		itemForm = new ConsignRenewItemUpdateForm(ui.getM().consignRenew_item_name());
