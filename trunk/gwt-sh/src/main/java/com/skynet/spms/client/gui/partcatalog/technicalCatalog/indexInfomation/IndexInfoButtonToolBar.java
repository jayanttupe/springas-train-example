...
public class IndexInfoButtonToolBar extends BaseButtonToolStript{
	...
	public IndexInfoButtonToolBar(String moduleName,IndexInfoListGrid indexInfoListGrid) {
		super(moduleName);
	...
	protected void showWindow(String buttonName, ToolStripButton button) {
		...
		if ("add".equalsIgnoreCase(buttonName)) {
			...
			if (indexInfoListGrid.getSelection().length == 1) {
				//useWindow = new IndexInfoListGridViewWindow(indexInfoListGrid);
				ShowWindowTools.showWondow(srcRect, useWindow, -1);
			} else {
				SC.say(ConstantsUtil.commonConstants.alertSelectFortoolbar());
				}
	...
	private void publishItem(String publishValue){
		...
		/*ListGridRecord[] records = indexInfoListGrid.getSelection();
		for(int i=0; i<records.length; i++){
			Record record = records[i];
			DynamicForm form = new DynamicForm();
			form.setDataSource(indexInfoListGrid.getDataSource());
			form.setValue("id", record.getAttribute("id"));
			form.setValue("publishStatus",publishValue);
			form.saveData();
		}*/
	}
