 ...
 * 
 */
public class LeaseContractAddWindow extends SuperWindow {
	...
	public LeaseContractAddWindow() {
		this.setTitle(ui.getB()
				.addFormTitle(ui.getM().mod_leaseContract_name()));
		/** 主面板 **/
	...
	private HLayout getShowGridView() {
		...
		Label leftLb = new Label(ui.getB().leaseRequisitionSheetItems());
		leftLb.setHeight("20");
		leftLayout.addMember(leftLb);// 先放label
		leftLayout.addMember(getLeftGrid());// 再放Grid

		VLayout rightLayout = new VLayout();
		Label rightLb = new Label(ui.getB().leaseContractItems());
	...
	private ListGrid getLeftGrid() {
		...
		lg.setDataSource(modelLocator.LeaseRequisitionSheetItemdataSource);
	...
	private VLayout getProcurementItemView() {
		...
		modelLocator.saveBtn.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				leaseContractId.setValue(modelLocator.LeaseContractId);
				itemLdf.saveData(new DSCallback() {
					public void execute(DSResponse response, Object rawData,
						...
						Criteria criteria = new Criteria();
						criteria
								.addCriteria("id", modelLocator.LeaseContractId);
						criteria.addCriteria("_r", String
								.valueOf(Math.random()));
						lg.fetchData(criteria);
						business
