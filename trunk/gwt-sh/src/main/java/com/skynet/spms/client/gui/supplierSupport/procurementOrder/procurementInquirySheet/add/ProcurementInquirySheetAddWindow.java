...
public class ProcurementInquirySheetAddWindow extends BaseWindow {
	...
	protected Canvas getLeftLayout(Rectangle srcRect, ListGrid listGrid) {
		this.setTitle(ui.getM().addFormTitle(ui.getM().mod_procurementInquiry_name()));
	...
	private VLayout getPrimaryView() {
		...
		v.setWidth100();
		v.setHeight100();
		v.setGroupTitle(ui.getM().mod_procurementInquiry_name());//新建采购询价单
		v.setIsGroup(true);
		...
		item_1.setValue(ui.getM().title_manufacturer());
	...
	private HLayout getShowGridView() {
		...
		//采购指令列表
		Label leftLb = new Label(ui.getM().msgListTitle(ui.getM().mod_procurementOrder_name()));
		leftLb.setHeight("20");
		...
		//采购询价单列表
		Label rightLb = new Label(ui.getM().msgListTitle(ui.getM().mod_procurementInquiry_name()));
	...
	private VLayout getItemView() {
		...
		// 构建操作按钮
		saveItemBtn = new IButton(ui.getM().btnSaveItem());//保存明细
		...
		saveItemBtn.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				if (procurementOrderLG.getSelectedRecord() != null
					...
					//请先选择采购指令明细项
					SC.say(ui.getM().mod_procurementOrder_msg_firstSelect());
				}
			}
		});
		// 清空按钮
