...
import com.skynet.spms.client.gui.supplierSupport.procurementOrder.suppliersParity.*;
 ...
 * 
 */
public class ProcurementContractAddWin extends BaseWindow {

	public ProcurementContractAddWin(String windowTitle, boolean isMax,
	...
	/** 合同明细项表单 **/
	public ContractItemAddForm procurementContractItemForm;
	...
	protected Canvas getLeftLayout(Rectangle srcRect, ListGrid listGrid) {
		...
		ContractAddBaseForm baseForm = new ContractAddBaseForm(
				...
				.addRecordClickHandler(new RecordClickHandler() {
					...
					public void onRecordClick(RecordClickEvent event) {
						...
						if (record != null) {
							...
							procurementContractItemForm.item_keys
									.setValue(record
							...
							procurementContractItemForm.deliveryTypeDate.setValue(record
		...
		final IButton viewBtn = new IButton("查看比价单");
		viewBtn.addClickHandler(new ClickHandler() {
			
			public void onClick(ClickEvent event) {
				SuppliersParityModel spmodel =SuppliersParityModel.getInstance();
				spmodel.contractId=model.contractID;
				if(!"".equals(spmodel.contractId)||spmodel.contractId!=null){
					SuppliersParityShowWindow win = new SuppliersParityShowWindow("比价单", true, viewBtn.getPageRect(), null, null);
					ShowWindowTools.showWindow(viewBtn.getPageRect(), win, 200);
				}else{
					SC.say("抱歉，比价单不存在");
				}
		...
		hlBtn.addMember(viewBtn);
		procurementContractItemGridLayout.addMember(hlBtn);
		
		mainLayout.addMember(gridLayout);
		procurementContractItemForm = new ContractItemAddForm("合同明细项",
				procurementContractItemGrid);
